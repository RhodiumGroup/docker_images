...
got.post(`https://api.travis-ci.org/repo/climateimpactlab%2Fclawpack/requests`, {
  headers: {
  ...
  body: JSON.stringify({
    request: {
      message: `Trigger build at ClimateImpactLab/clawpack commit: ${gitCommitHash}`,
      branch: 'source',
    },    
  }),
})
.then(() => {
  console.log("Triggered build of ClimateImpactLab/clawpack");
})
.catch((err) => {
  console.error(err);
