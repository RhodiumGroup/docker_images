...
if [[ -e "/opt/app/environment.yml" ]]; then
    ...
    /opt/conda/bin/conda env update -n octave -f /opt/app/environment.yml
else
...
if [[ "$EXTRA_CONDA_PACKAGES" ]]; then
    ...
    /opt/conda/bin/conda install -n octave --yes $EXTRA_CONDA_PACKAGES
fi

if [[ "$EXTRA_PIP_PACKAGES" ]]; then
    echo "EXTRA_PIP_PACKAGES environment variable found.  Installing".
    /opt/conda/envs/octave/bin/pip install $EXTRA_PIP_PACKAGES
fi
